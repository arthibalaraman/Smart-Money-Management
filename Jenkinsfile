        ...
        stage('Cleanup') {
            steps {
                ...
                sh '''
                    ...
                    # 2. Stop ANY docker container using our target ports (3080, 5000)
                    echo "🔍 Checking for any other containers on ports 3080/5000..."
                    docker ps -q --filter "publish=3080" | xargs -r docker stop || true
                    docker ps -q --filter "publish=5000" | xargs -r docker stop || true
                    
                    # 3. Force kill any remaining host processes on these ports
                    for port in 3080 5000; do
